...
from utils import natural_key  # [V204] 引用通用工具

# --- 内部辅助函数: KPI 卡片 (保留，因仅本页使用) ---
...
# --- 内部辅助函数: 转售数据处理 (保留，因逻辑较复杂且专用于此页) ---
def _process_resale_data(df):
    # 确保必要的列存在，否则直接返回空
    if 'Unit_ID' not in df.columns: 
        # 尝试通过 utils 标准化后的列构建
        if all(c in df.columns for c in ['BLK', 'Stack', 'Floor_Num']):
            df['_uid'] = df['BLK'].astype(str) + "-" + df['Stack'].astype(str) + "-" + df['Floor_Num'].astype(str)
        else:
            return pd.DataFrame()
    else:
        df['_uid'] = df['Unit_ID']
        
    df_sorted = df.sort_values(['_uid', 'Sale Date'])
...
def render(df, chart_color='#1f77b4', chart_font_size=12, inventory_map=None):
    ...
    if df is None or df.empty:
        st.error("暂无数据"); return

    # 确保年份列存在 (app.py 的 load_data 可能已经处理，但为了安全再检查一次)
    if 'Year' not in df.columns and 'Sale Date' in df.columns:
    ...
    # 兼容 utils.py 清洗后的 'Type' 或 'Category'
    ...
    with tab_trend_all:
        yearly = df.groupby('Year').agg({'Sale Price': 'count', 'Unit Price ($ psf)': 'mean'}).reset_index()
        fig = go.Figure()
        fig.add_trace(go.Bar(x=yearly['Year'], y=yearly['Sale Price'], name='成交量', marker_color='#dbeafe', yaxis='y'))
        fig.add_trace(go.Scatter(x=yearly['Year'], y=yearly['Unit Price ($ psf)'], name='平均尺价', mode='lines+markers', line=dict(color=chart_color, width=3), yaxis='y2'))
    ...
    with tab_trend_cat:
        if cat_col:
            cat_trend = df.groupby(['Year', cat_col]).agg({'Unit Price ($ psf)': 'mean'}).reset_index()
            fig_cat = px.line(cat_trend, x='Year', y='Unit Price ($ psf)', color=cat_col, markers=True, title="各户型平均尺价走势")
    ...
    if resale_df.empty:
        ...
        with c3: st.markdown(kpi_card("单位最大转售", f"{max_turnover} 次", f"平均周转: {avg_turnover:.1f} 次"), unsafe_allow_html=True)
