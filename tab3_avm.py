...
import io
import urllib.parse
import utils_address # [V192] 引入地址管理模块
...
# ==========================================
# 🛠️ DATA CLEANING & HELPER FUNCTIONS
# ==========================================
...
def generate_pdf_letter(project_name, blk, floor, stack, area, u_type, est_price, est_psf, comps_df, mailing_address, last_price=0, last_date=None):
    ...
    if last_price > 0 and last_date is not None:
        ...
        profit_text = f"Records indicate this unit was last purchased on <b>{last_date.strftime('%Y-%m-%d')}</b> for <b>${last_price/1e6:.2f}M</b>. Based on our current valuation, your estimated gross capital appreciation is:<br/><br/><b><font color='green' size='12'>+${profit/1e6:.2f} Million ({profit_pct:.1f}%)</font></b><br/><br/>This represents a significant return over the past {years_held:.1f} years."
    ...
    with st.expander("⚙️ 调整参数 (Calibration)", expanded=True):
        ...
        with c_cal1:
            input_area = st.number_input(
                "面积 (sqft)", value=input_area, step=10, key=f"cal_area_{widget_key_suffix}"
            )
        with c_cal2:
            input_type = st.selectbox(
                "户型 (Type)", options=all_types, index=all_types.index(input_type) if input_type in all_types else 0, key=f"cal_type_{widget_key_suffix}"
            )
    ...
    with c1:
        ...
        if last_price > 0:
            ...
            <div style="margin-top:5px; margin-bottom:10px; font-size:14px; color:#15803d; font-weight:bold;">
                📈 预计增值: +${profit/1e6:.2f}M ({profit_pct:.1f}%)
            </div>
            """, unsafe_allow_html=True)
