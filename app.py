...
with st.sidebar:
    ...
    if df is not None:
        cat_ops = ["按户型面积段 (自动分箱)", "按楼座 (Block)", "按卧室数量 (Bedroom Type)"]
        for c in ['Bedroom Type', 'Bedrooms', 'Type']:
            if c in df.columns: cat_ops.insert(0, "按卧室数量 (Bedroom Type)"); break
...
if df is not None:
    ...
    with t1: tab1_market.render(df, chart_color, chart_font_size, inv_map)
